...
public class IterableWithPolicyImpl<T> implements IterableWithPolicy<T> {
    ...
    public IterableWithPolicyImpl(T[] elements, Predicate<T> predicate){
        this.elOFArray = Arrays.copyOf(elements, elements.length);
    ...
    private class SimpleIterator implements Iterator<T>{
        ...
        public boolean hasNext() {  
            exist = true;
            this.incrCounter();  
            if(counter >= IterableWithPolicyImpl.this.getArraySize()){
                return false;
            } 
            current = IterableWithPolicyImpl.this.getElementByIndex(counter); 
            if(!IterableWithPolicyImpl.this.filtering.test(current)){
                exist = this.hasNext();
            }
            return exist;
        }
